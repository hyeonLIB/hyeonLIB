...
def update_data(selected_datas):
    ...
    for selected_data in selected_datas:
        ...
        df = df.loc[:15] # Need to remove
        df_viewer = pd.concat([df_viewer, df], ignore_index=True)
...
def viewer_update(n_clicks, selected_columns):
    if n_clicks is None:
        ...
        if selected_columns == []:
            df_viewer = main_dataframe.loc[:, ~main_dataframe.columns.str.contains("wt_")] # need to remove (This line is for programming speed)
            columns = [{'name': i, 'id':i, 'deletable':True} for i in df_viewer.columns]
            ...
            for q in query_list:
                ...
                if q['cond'] == '=':
                    if 'float' not in str(type(main_dataframe[q['var']][2])): # str
                        val = q['val']
                        filtered_data = filtered_data.query(f'{q["var"]} == "{val}"')
                        val_count = len(filtered_data)
                    else: # float

...
def sync_callback(selected_columns, options, slctd_row_indices, all_rows_data, selected_data, n_clicks, n_clicks_col_type, query):
    ctx = dash.callback_context
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]
    print(ctx)
    print(list(ctx.triggered[0].keys())[1])
    # dpdn-col-selection
    ...
    # dpdn-data-selection
        ...
        return selected_columns, slctd_row_indices, options, selected_data
    ...
    return selected_columns, slctd_row_indices, no_update, no_update

# @app.callback(  ###################### Warning 
...
def cell_clicked(active_cell, columns_data, is_open):
    ctx = dash.callback_context
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]

    if trigger_id == 'modal-properties-specification':
        if not is_open:
            return active_cell, columns_data, [], None
        else:
            return active_cell, columns_data, no_update, no_update
    elif trigger_id == 'table-col-selection':
        ...
        if active_cell["column_id"] == 'col_type':
            ...
            if col_type == 'categorical':
                ...
                return modal_children, no_update, 'xl', {'col_name':selected_column,'col_type':'categorical'}
        ...
        # Numeric variable
            ...
            return modal_children, no_update, 'lg', {'col_name':selected_column,'col_type':'numeric'}, no_update, no_update
    else:
...
#     Input(component_id='checklist-data-selection', component_property='value'),
# )
# def update_data(selected_datas, ch_selected_datas):
#     """
...
#         return selected_datas, columns, df_viewer.to_dict('records'), dpdn_col_selection, [], sel_table_data, ch_selected_datas

#     elif trigger_id == "checklist-data-selection":
#         return no_update, no_update, no_update, no_update, no_update, no_update, ch_selected_datas
...
#     State(component_id="local", component_property="data"),
# )
# def change_col_type(n_clicks, changed_col):
#     if n_clicks == 0:
#         raise PreventUpdate
#     else:
#         global main_dataframe
#         if changed_col['col_type'] == 'categorical':
#             main_dataframe = main_dataframe.astype({changed_col['col_name']:np.float64})
#         else:
#             main_dataframe = main_dataframe.astype({changed_col['col_name']:'int32'})
#             main_dataframe = main_dataframe.astype({changed_col['col_name']:'str'})
